...
function(a){for(var a=a.split("."),b=window,c=0;c<a.length;c++)b[a[c]]||(b[a[c]]={}),b=b[a[c]]};GeoGlobe.VERSION_NUMBER="GeoGlobeJSAPI 2019 -- $Version: 0.53.1 build-2020/8/31 9:00:467 $"})();GeoGlobe.Class=function(){var a=arguments.length,b=arguments[0],c=arguments[a-1],d=typeof c.initialize=="function"?c.initialize:function(){b.prototype.initialize.apply(this,arguments)};a>1?[d,b].concat(Array.prototype.slice.call(arguments).slice(1,a-1),c):d.prototype=c;return d};
...
GeoGlobe.Util.distVincenty=function(a,b){for(var c=GeoGlobe.Util.VincentyConstants,d=c.a,e=c.b,c=c.f,f=GeoGlobe.Util.rad(b.lng-a.lng),g=Math.atan((1-c)*Math.tan(GeoGlobe.Util.rad(a.lat))),h=Math.atan((1-c)*Math.tan(GeoGlobe.Util.rad(b.lat))),j=Math.sin(g),g=Math.cos(g),l=Math.sin(h),h=Math.cos(h),m=f,o=2*Math.PI,p=20;Math.abs(m-o)>1.0E-12&&--p>0;){var r=Math.sin(m),n=Math.cos(m),q=Math.sqrt(h*r*h*r+(g*l-j*h*n)*(g*l-j*h*n));if(q==0)return 0;var n=j*l+g*h*n,t=Math.atan2(q,n),s=Math.asin(g*h*r/q),u=
Math.cos(s)*Math.cos(s),r=n-2*j*l/u,v=c/16*u*(4+c*(4-3*u)),o=m,m=f+(1-v)*c*Math.sin(s)*(t+v*q*(r+v*n*(-1+2*r*r)))}if(p==0)return NaN;d=u*(d*d-e*e)/(e*e);c=d/1024*(256+d*(-128+d*(74-47*d)));return(e*(1+d/16384*(4096+d*(-768+d*(320-175*d))))*(t-c*q*(r+c/4*(n*(-1+2*r*r)-c/6*r*(-3+4*q*q)*(-3+4*r*r))))).toFixed(3)/1E3};
GeoGlobe.Util.destinationVincenty=function(a,b,c){for(var d=GeoGlobe.Util,e=d.VincentyConstants,f=e.a,g=e.b,e=e.f,h=a.lng,j=a.lat,a=d.rad(b),b=Math.sin(a),a=Math.cos(a),l=(1-e)*Math.tan(d.rad(j)),j=1/Math.sqrt(1+l*l),m=l*j,o=Math.atan2(l,a),l=j*b,p=1-l*l,f=p*(f*f-g*g)/(g*g),r=1+f/16384*(4096+f*(-768+f*(320-175*f))),n=f/1024*(256+f*(-128+f*(74-47*f))),f=c/(g*r),q=2*Math.PI;Math.abs(f-q)>1.0E-12;)var t=Math.cos(2*o+f),s=Math.sin(f),u=Math.cos(f),v=n*s*(t+n/4*(u*(-1+2*t*t)-n/6*t*(-3+4*s*s)*(-3+4*t*t))),
q=f,f=c/(g*r)+v;c=m*s-j*u*a;c=Math.atan2(m*u+j*s*a,(1-e)*Math.sqrt(l*l+c*c));g=e/16*p*(4+e*(4-3*p));return new GeoGlobe.LngLat(h+d.deg(Math.atan2(s*b,j*u-m*s*a)-(1-g)*e*l*(f+g*s*(t+g*u*(-1+2*t*t)))),d.deg(c))};
...
b.prototype=a.prototype;if(g&&f.wrapped)b.wrapped=f.wrapped;b.UNSENT=0;b.OPENED=1;b.HEADERS_RECEIVED=2;b.LOADING=3;b.DONE=4;b.prototype.readyState=b.UNSENT;b.prototype.responseText="";b.prototype.responseXML=null;b.prototype.status=0;b.prototype.statusText="";b.prototype.priority="NORMAL";b.prototype.onreadystatechange=null;b.onreadystatechange=null;b.onopen=null;b.onsend=null;b.onabort=null;b.prototype.open=function(a,f,j,p,r){delete this._headers;arguments.length<3&&(j=!0);this._async=j;var n=this,
q=this.readyState,t;h&&j&&(t=function(){q!=b.DONE&&(e(n),n.abort())},window.attachEvent("onunload",t));b.onopen&&b.onopen.apply(this,arguments);arguments.length>4?this._object.open(a,f,j,p,r):arguments.length>3?this._object.open(a,f,j,p):this._object.open(a,f,j);try{this._object.responseType="msxml-document"}catch(s){}this.readyState=b.OPENED;c(this);this._object.onreadystatechange=function(){if(!g||j)n.readyState=n._object.readyState,d(n),n._aborted?n.readyState=b.UNSENT:(n.readyState==b.DONE&&(delete n._data,
...
this._isMoving=this._MAPBOX_MOVE;this._isSync=!1;this._currentCesiumHeading=0;this.cameraToCenterDistance=1405.5;this._currentCesiumPitch=-Cesium.Math.PI_OVER_TWO;this._handler=new Cesium.ScreenSpaceEventHandler(this.viewer.canvas);this._bindEvent();this._updateCesium()},_bindEvent:function(){if(this.map&&!this._handler.getInputAction(Cesium.ScreenSpaceEventType.MOUSE_MOVE)){var a=this;this.map.on("move",function(){a._isSync&&a._updateCesium()});this.map.on("mouseover",function(){a._isMoving=a._MAPBOX_MOVE});
...
destroy:function(){if(this.map)this._unbindEvent(),this.viewer=this._handler=this.map=null},_calDistanceFromMapboxToCesium:function(){var a=78271.51696402031/Math.pow(2,this.map.getZoom()+1.2),b=this.viewer.scene.globe.getHeight(new Cesium.Cartographic(Cesium.Math.toRadians(this.map.getCenter().lng),Cesium.Math.toRadians(this.map.getCenter().lat)))||0;return this.cameraToCenterDistance*a+b},_calHeadingFromMapboxToCesium:function(){var a=this.map.getBearing();return Cesium.Math.toRadians(a)},_calPitchFromMapboxToCesium:function(){var a=
...
_updateCesium:function(){if(this._isMoving!==this._CESIUM_MOVE){var a=this._calDistanceFromMapboxToCesium(),b,c;c=this.isHeadingChanged?this._calHeadingFromMapboxToCesium():this._currentCesiumHeading;b=this.isPitchChanged?this._calPitchFromMapboxToCesium():this._currentCesiumPitch;this.viewer.camera.setView({destination:this._calDestinationFromMapboxToCesium(),orientation:{heading:c,pitch:b}});this.viewer.camera.moveBackward(a)}},_updateMapbox:function(){if(this.viewer&&this._isMoving!==this._MAPBOX_MOVE){var a=
Cesium.Ellipsoid.WGS84,b=0,c=0,c=this._getCenterPoint();if(!c)b=this.viewer.scene.globe,c=this.viewer.camera.positionCartographic.clone(),b=b.getHeight(c),c.height=b||0,c=a.cartographicToCartesian(c);var b=Cesium.Cartesian3.distance(c,this.viewer.camera.position)/this.cameraToCenterDistance,b=Math.log2(78271.51696402031/b)-1.2,d=a.cartesianToCartographic(c),d=[Cesium.Math.toDegrees(d.longitude),Cesium.Math.toDegrees(d.latitude)];this.map.setZoom(b);this.map.setCenter(d);if(c){var b=this.viewer.camera.up,
d=this.viewer.camera.right,e=new Cesium.Cartesian3(-c.y,c.x,0),d=Cesium.Cartesian3.angleBetween(d,e),b=Cesium.Cartesian3.cross(c,b,new Cesium.Cartesian3).z<0?-d:d,b=Cesium.Math.toDegrees(b),d=this.viewer.camera.position,e=new Cesium.Cartesian3;a.geocentricSurfaceNormal(c,e);a=new Cesium.Cartesian3;Cesium.Cartesian3.subtract(d,c,a);Cesium.Cartesian3.normalize(a,a);a=Math.acos(Cesium.Cartesian3.dot(e,a));c=isNaN(a)?0:a}else b=Cesium.Math.toDegrees(this.viewer.camera.heading),c=Cesium.Math.PI_OVER_THREE;
this.isHeadingChanged&&this.map.setBearing(b);this.isPitchChanged&&this.map.setPitch(Cesium.Math.toDegrees(c))}},CLASS_NAME:"GeoGlobe.View2D.Control.MapCesium"});GeoGlobe.Event=mapboxgl.Evented;GeoGlobe.Event.MapMouseEvent=mapboxgl.Event;GeoGlobe.Event.MapTouchEvent=mapboxgl.Event;GeoGlobe.Event.mapDataEvent=mapboxgl.Event;GeoGlobe.Event.mapBoxZoomEvent=mapboxgl.Event;GeoGlobe.Handler=GeoGlobe.Class({initialize:function(){},CLASS_NAME:"GeoGlobe.Handler"});
...
{x1:d.x,y1:d.y,x2:c.x,y2:c.y};return b.sort(function(a,b){return a.x1-b.x1})},splitWithSegment:function(a,b){for(var c=!(b&&b.edge===!1),d=b&&b.tolerance,e=[],f=this.getVertices(),g=[],h=[],j=!1,l,m,o,p={point:!0,tolerance:d},r=null,n=0,q=f.length-2;n<=q;++n)if(d=f[n],g.push(d.clone()),l=f[n+1],m={x1:d.x,y1:d.y,x2:l.x,y2:l.y},m=GeoGlobe.Geometry.segmentsIntersect(a,m,p),m instanceof GeoGlobe.Geometry.Point&&((o=m.x===a.x1&&m.y===a.y1||m.x===a.x2&&m.y===a.y2||m.equals(d)||m.equals(l)?!0:!1)||c))m.equals(h[h.length-
1])||h.push(m.clone()),!(n===0&&m.equals(d))&&!m.equals(l)&&(j=!0,m.equals(d)||g.push(m),e.push(new GeoGlobe.Geometry.LineString(g)),g=[m.clone()]);j&&(g.push(l.clone()),e.push(new GeoGlobe.Geometry.LineString(g)));if(h.length>0)var t=a.x1<a.x2?1:-1,s=a.y1<a.y2?1:-1,r={lines:e,points:h.sort(function(a,b){return t*a.x-t*b.x||s*a.y-s*b.y})};return r},split:function(a,b){var c=null,d=b&&b.mutual,e,f,g,h;if(a instanceof GeoGlobe.Geometry.LineString){var j=this.getVertices(),l,m,o,p,r,n=[];g=[];for(var q=
0,t=j.length-2;q<=t;++q){l=j[q];m=j[q+1];o={x1:l.x,y1:l.y,x2:m.x,y2:m.y};h=h||[a];d&&n.push(l.clone());for(var s=0;s<h.length;++s)if(p=h[s].splitWithSegment(o,b))if(r=p.lines,r.length>0&&(r.unshift(s,1),Array.prototype.splice.apply(h,r),s+=r.length-2),d)for(var u=0,v=p.points.length;u<v;++u)r=p.points[u],r.equals(l)||(n.push(r),g.push(new GeoGlobe.Geometry.LineString(n)),n=r.equals(m)?[]:[r.clone()])}d&&g.length>0&&n.length>0&&(n.push(m.clone()),g.push(new GeoGlobe.Geometry.LineString(n)))}else c=
...
GeoGlobe.Geometry.MultiLineString=GeoGlobe.Class4OL(GeoGlobe.Geometry.Collection,{componentTypes:["GeoGlobe.Geometry.LineString"],split:function(a,b){for(var c=null,d=b&&b.mutual,e,f,g,h,j=[],l=[a],m=0,o=this.components.length;m<o;++m){f=this.components[m];g=!1;for(var p=0;p<l.length;++p)if(e=f.split(l[p],b)){if(d){g=e[0];for(var r=0,n=g.length;r<n;++r)r===0&&j.length?j[j.length-1].addComponent(g[r]):j.push(new GeoGlobe.Geometry.MultiLineString([g[r]]));g=!0;e=e[1]}if(e.length){e.unshift(p,1);Array.prototype.splice.apply(l,
...
p+=g.length-2),e=e[1],e.length===0&&(e=[f.clone()]));g=0;for(var r=e.length;g<r;++g)g===0&&l.length?l[l.length-1].addComponent(e[g]):l.push(new GeoGlobe.Geometry.MultiLineString([e[g]]));g=!0}g||(l.length?l[l.length-1].addComponent(f.clone()):l=[new GeoGlobe.Geometry.MultiLineString([f.clone()])])}}else c=a.split(this);j&&j.length>1?h=!0:j=[];l&&l.length>1?g=!0:l=[];if(h||g)c=d?[j,l]:l;return c},CLASS_NAME:"GeoGlobe.Geometry.MultiLineString"});
...
a.features?a.features.length:0,c=Array(b),e=0;e<b;++e)c[e]=a.features[e].fid;n.insertIds=c;d.apply(this,[a])}function d(a){this.callUserCallback(a,b);r=r&&a.success();f++;if(f>=p&&b.callback)n.code=r?GeoGlobe.Protocol.Response.SUCCESS:GeoGlobe.Protocol.Response.FAILURE,b.callback.apply(b.scope,[n])}var b=GeoGlobe.Util.applyDefaults(b,this.options),e=[],f=0,g={};g[GeoGlobe.State.INSERT]=[];g[GeoGlobe.State.UPDATE]=[];g[GeoGlobe.State.DELETE]=[];for(var h,j,l=[],m=0,o=a.length;m<o;++m)if(h=a[m],j=g[h.state])j.push(h),
l.push(h);var p=(g[GeoGlobe.State.INSERT].length>0?1:0)+g[GeoGlobe.State.UPDATE].length+g[GeoGlobe.State.DELETE].length,r=!0,n=new GeoGlobe.Protocol.Response({reqFeatures:l});h=g[GeoGlobe.State.INSERT];h.length>0&&e.push(this.create(h,GeoGlobe.Util.applyDefaults({callback:c,scope:this},b.create)));h=g[GeoGlobe.State.UPDATE];for(m=h.length-1;m>=0;--m)e.push(this.update(h[m],GeoGlobe.Util.applyDefaults({callback:d,scope:this},b.update)));h=g[GeoGlobe.State.DELETE];for(m=h.length-1;m>=0;--m)e.push(this["delete"](h[m],
...
(g[o.identifier]=o["default"])}for(var p=c.projection||h.supportedCRS.replace(/urn:ogc:def:crs:(\w+):(.*:)?(\w+)$/,"$1:$3"),r=c.units||(p==="EPSG:4326"?"degrees":"m"),n=[],q,t,c=[],m=e.tileMatrixSetLinks,g=function(a){n.push(a*2.8E-4/GeoGlobe.METERS_PER_INCH/GeoGlobe.INCHES_PER_UNIT[r]);if(!q||q>a)q=a;if(!t||t<a)t=a},o=0,j=m.length;o<j;o++)if(f=m[o],f.tileMatrixSet===h.identifier){if(f.tileMatrixSetLimits){for(var j={},o=0,s=h.matrixIds.length;o<s;o++)j[h.matrixIds[o].identifier]=h.matrixIds[o];o=
...
(c="KVP");var m=[],e=b.params||{};delete b.params;f=0;for(j=d.dimensions.length;f<j;f++){var o=d.dimensions[f];m.push(o.identifier);e.hasOwnProperty(o.identifier)||(e[o.identifier]=o["default"])}for(var m=b.projection||h.supportedCRS.replace(/urn:ogc:def:crs:(\w+):(.*:)?(\w+)$/,"$1:$3"),p=b.units||(m==="EPSG:4326"?"degrees":"m"),r=[],n,q,m=[],o=d.tileMatrixSetLinks,f=function(a){r.push(a*2.8E-4/GeoGlobe.METERS_PER_INCH/GeoGlobe.INCHES_PER_UNIT[p]);if(!n||n>a)n=a;if(!q||q<a)q=a},t=0,j=o.length;t<j;t++)if(e=
...
b[c]instanceof Function?!0:!1}function o(a){var b=0;if(a instanceof Object)for(var c in a)m(a,c)||b++;return b}function p(b){var c=[];if(b instanceof Object)for(var d in b)d.toString().indexOf("__")==-1&&d.toString().indexOf(a.attributePrefix)==0&&c.push(d);return c}function r(b){var d="";if(b instanceof Object){var e="";b.__cdata!=null&&(e+="<![CDATA["+b.__cdata+"]]\>");b.__text!=null&&(e+=a.escapeMode?c(b.__text):b.__text);d+=e}else b!=null&&(d+=a.escapeMode?c(b):b);return d}function n(b,c){var e=
...
null},addTo:function(a){this._three=a;this._three.addLayer(this)},remove:function(){this._three.removeLayer(this.id)},render:function(){var a=this;this._three._removeInnerLayer(this);this._meshes=[];if(a.timer)cancelAnimationFrame(a.timer),a.timer=null;for(var b=[],c=[],d=[],e=[],f=0;f<this.data.length;f++){var g=this.data[f],h=this.getWidth(g),j=new THREE.Color(this.getColor(g)),l=new THREE.LineMaterial({linewidth:5.0E-4*h,color:j,opacity:1,transparent:!0}),m=[],o=[],p=[],r=this.data[f].geometry.coordinates;
if(r.length>1)var n=this._three._threebox.projectToWorld(r[0]),q=this._three._threebox.projectToWorld(r[r.length-1]),q=Math.sqrt(Math.pow(q.x-n.x,2)+Math.pow(q.y-n.y,2)+Math.pow(q.z-n.z,2));for(var t=0;t<r.length-1;t++){var m=this._three._threebox.projectToWorld(r[t]),s=this._three._threebox.projectToWorld(r[t+1]),u={x:(m.x+s.x)/2,y:(m.y+s.y)/2,z:Math.sqrt(Math.pow(s.x-m.x,2)+Math.pow(s.y-m.y,2))/2*Math.tan(Math.PI/2*this.getCurveness(g))},u=new THREE.QuadraticBezierCurve3(new THREE.Vector3(m.x,m.y,
m.z),new THREE.Vector3(u.x,u.y,u.z),new THREE.Vector3(s.x,s.y,s.z));if(r.length>1)u.rate1=Math.sqrt(Math.pow(m.x-n.x,2)+Math.pow(m.y-n.y,2)+Math.pow(m.z-n.z,2))/q,u.rate2=Math.sqrt(Math.pow(s.x-m.x,2)+Math.pow(s.y-m.y,2)+Math.pow(s.z-m.z,2))/q;p.push(u);m=u.getPoints(50);for(s=0;s<m.length;s++)o.push(m[s].x,m[s].y,m[s].z)}r=new THREE.LineGeometry;r.setPositions(o);l=new THREE.Line2(r,l);l.computeLineDistances();l.visible=this.visible;l.name=(this.id?this.id:"threelayer")+"-"+f;l.userData.attributes=
...
b.name="particles";b.userData.attributes={OriginalData:g,Layer:this};this._three._threebox.addGeoreferencedMesh(b);this._meshes.push(b);this._three._render();var w=0,y;(function B(){a.timer=requestAnimationFrame(B);w+=0.005;w>1&&(w=0);for(var b=v.attributes.position.array,c=0;c<e.length;c++){if(e[c].length===1)y=e[c][0].getPoint(w);else for(var d=0;d<e[c].length;d++)if(w>=e[c][d].rate1&&w<=e[c][d].rate1+e[c][d].rate2){y=e[c][d].getPoint((w-e[c][d].rate1)/e[c][d].rate2);break}b[c*3]=y.x;b[c*3+1]=y.y;
b[c*3+2]=y.z}v.attributes.position.needsUpdate=!0;a._three._render()})()}});
...
!0;this.getColor=a.getColor?a.getColor:this.getColor;this.getPolygon=a.getPolygon?a.getPolygon:this.getPolygon;this.getElevation=a.getElevation?a.getElevation:this.getElevation;this.getBaseElevation=a.getBaseElevation?a.getBaseElevation:this.getBaseElevation;this.timer=null}else console.error("\u672a\u5f15\u5165turf.js\u5e93\uff01")},addTo:function(a){this._three=a;this._three.addLayer(this)},remove:function(){this._three.removeLayer(this.id)},_createMaterial:function(){},_createLineGeometry:function(a){var b=
new THREE.Geometry;a.forEach(function(a){b.vertices.push(a)});return b},_createGeoExtrudeGeometry:function(a,b){var c=new THREE.Shape(a),c=new THREE.GeoExtrudeGeometry(c,b),d=new THREE.Geometry;d.faceVertexUvs=c.faceVertexUvs.concat();d.faces=c.faces.concat();d.vertices=c.vertices.concat();return d},_createSideGeometry:function(a,b){var c=new THREE.Shape(a),c=new THREE.GeoExtrudeGeometry(c,b),d=0;c.faces.forEach(function(a){a.normal.z!=0&&d++});c.faces.splice(0,d);var e=new THREE.Geometry;e.faceVertexUvs=
c.faceVertexUvs.concat();e.faces=c.faces.concat();e.vertices=c.vertices.concat();return e},_createPillarMesh:function(a,b,c){var d=this,e=new THREE.Geometry;a.forEach(function(a){var g=[];a.geometry.coordinates[0].forEach(function(a){g.push(d._three._threebox.projectToWorld(a))});var a=new THREE.Shape(g),a=new THREE.GeoExtrudeGeometry(a,b),h=new THREE.Geometry;h.faceVertexUvs=a.faceVertexUvs.concat();h.faces=a.faces.concat();h.vertices=a.vertices.concat();a=new THREE.Mesh(h,c);e.merge(a.geometry,
a.matrix)});return new THREE.Mesh(e,c)},highlightFloor_Wall:function(a){this.isClicked=!0;if(this._INTERSECTED_FLOOR_3&&this._INTERSECTED_WALL_2)this._INTERSECTED_FLOOR_3.material.color=new THREE.Color(this.originalFloorColor),this._INTERSECTED_FLOOR_3.material.opacity=this.originalFloorOpacity,this._INTERSECTED_WALL_2.material.color=new THREE.Color(this.originalWallColor),this._INTERSECTED_WALL_2.material.opacity=this.originalWallOpacity;var b=null,c=null;if(a.length>0){for(var d=0;d<a.length;d++){if(a[d].object.userData.tag==
"__wall__")c=a[d].object;if(a[d].object.userData.tag=="__floor__"){b=a[d].object;break}}if(b&&c&&this._INTERSECTED_FLOOR!=b&&this._INTERSECTED_WALL!=c){if(this._INTERSECTED_FLOOR&&this._INTERSECTED_WALL)this._INTERSECTED_FLOOR.material.color=new THREE.Color(this.originalFloorColor),this._INTERSECTED_FLOOR.material.opacity=this.originalFloorOpacity,this._INTERSECTED_WALL.material.color=new THREE.Color(this.originalWallColor),this._INTERSECTED_WALL.material.opacity=this.originalWallOpacity;this._INTERSECTED_FLOOR=
b;this._INTERSECTED_FLOOR.currentHex=this._INTERSECTED_FLOOR.material.color.getHex();this._INTERSECTED_FLOOR.currentOpacity=this._INTERSECTED_FLOOR.material.opacity;this._INTERSECTED_FLOOR.material.color=new THREE.Color(this.floorColor);this._INTERSECTED_FLOOR.material.opacity=this.floorColorOpacity;this._INTERSECTED_WALL=c;this._INTERSECTED_WALL.currentHex=this._INTERSECTED_WALL.material.color.getHex();this._INTERSECTED_WALL.currentOpacity=this._INTERSECTED_WALL.material.opacity;this._INTERSECTED_WALL.material.color=
new THREE.Color(this.wallColor);this._INTERSECTED_WALL.material.opacity=this.wallColorOpacity;this._three._render()}}else this._INTERSECTED_WALL=this._INTERSECTED_FLOOR=null;return b},highlightFloor:function(a){var b=null;if(a.length>0){for(var c=0;c<a.length;c++)if(a[c].object.userData.tag=="__floor__"){b=a[c].object;break}if(b&&this._INTERSECTED_FLOOR_2!=b){if(this._INTERSECTED_FLOOR_2)this._INTERSECTED_FLOOR_2.material.color.setHex(this._INTERSECTED_FLOOR_2.currentHex),this._INTERSECTED_FLOOR_2.material.opacity=
this._INTERSECTED_FLOOR_2.currentOpacity;if(this._INTERSECTED_FLOOR){if(this.isClicked)this._INTERSECTED_FLOOR.material.color=new THREE.Color(this.floorColor),this._INTERSECTED_FLOOR.material.opacity=this.floorColorOpacity;if(!this.isClicked)this._INTERSECTED_FLOOR_3.material.color=new THREE.Color(this.specificFloorColor),this._INTERSECTED_FLOOR_3.material.opacity=this.specificFloorColorOpacity}this._INTERSECTED_FLOOR_2=b;this._INTERSECTED_FLOOR_2.currentHex=this._INTERSECTED_FLOOR_2.material.color.getHex();
this._INTERSECTED_FLOOR_2.currentOpacity=this._INTERSECTED_FLOOR_2.material.opacity;this._INTERSECTED_FLOOR_2.material.color=new THREE.Color(this.movingFloorColor);this._INTERSECTED_FLOOR_2.material.opacity=this.movingFloorColorOpacity;this._three._render()}}else this._INTERSECTED_FLOOR_2=null;return b},locateToSpecificFloor:function(a){this.isClicked=!1;if(this._INTERSECTED_FLOOR&&this._INTERSECTED_WALL)this._INTERSECTED_FLOOR.material.color=new THREE.Color(this.originalFloorColor),this._INTERSECTED_FLOOR.material.opacity=
this.originalFloorOpacity,this._INTERSECTED_WALL.material.color=new THREE.Color(this.originalWallColor),this._INTERSECTED_WALL.material.opacity=this.originalWallOpacity;var b=this._three._threebox.world2.getObjectByName("this is "+a+" floor"),a=this._three._threebox.world2.getObjectByName("this is "+a+" wall");if(b&&a){if(this._INTERSECTED_FLOOR_3&&this._INTERSECTED_WALL_2&&this._INTERSECTED_FLOOR_3!=b&&this._INTERSECTED_WALL_2!=a)this._INTERSECTED_FLOOR_3.material.color=new THREE.Color(this.originalFloorColor),
this._INTERSECTED_FLOOR_3.material.opacity=this.originalFloorOpacity,this._INTERSECTED_WALL_2.material.color=new THREE.Color(this.originalWallColor),this._INTERSECTED_WALL_2.material.opacity=this.originalWallOpacity;this._INTERSECTED_FLOOR_3=b;this._INTERSECTED_FLOOR_3.material.color=new THREE.Color(this.specificFloorColor);this._INTERSECTED_FLOOR_3.material.opacity=this.specificFloorColorOpacity;this._INTERSECTED_WALL_2=a;this._INTERSECTED_WALL_2.material.color=new THREE.Color(this.specificWallColor);
this._INTERSECTED_WALL_2.material.opacity=this.specificWallColorOpacity;this._three._render();return b}},getSelectedFloor:function(a){var b=null;if(a.length>0)for(var c=0;c<a.length;c++)if(a[c].object.userData.tag=="__floor__"){b=a[c].object;break}return b},render:function(){function a(c,d){d>1E3/60&&(A<1?(C.position.copy(B.getPointAt(A)),A+=0.01):A=0,b._three._render(),d=0);b.timer=window.requestAnimationFrame(function(b){return a(b,d+b-c)})}var b=this;b._three._removeInnerLayer(b);b._meshes=[];
if(b.timer)cancelAnimationFrame(b.timer),b.timer=null;var c=new THREE.Group;c.visible=this.visible;c.name="main part";c.userData.attributes={OriginalData:b.data,Layer:b};var d=new THREE.Group;d.visible=this.visible;d.name="extra part";d.userData.attributes={OriginalData:b.data,Layer:b};if(b.data.geometry){var e=turf.centroid(b.data).geometry.coordinates;if(b.isCirclesVisible){var f=new THREE.MeshBasicMaterial({color:7372944,transparent:!0,opacity:0.3});[0.01,0.015,0.02,0.025,0.03,0.035,0.04,0.045,
0.05,0.055].forEach(function(a){var a=turf.circle(e,a,{steps:50,units:"kilometers",properties:{}}),d=new THREE.Geometry;a.geometry.coordinates[0].forEach(function(a){d.vertices.push(b._three._threebox.projectToWorld([a[0],a[1],-1]))});a=new THREE.Line(d,f);c.add(a)})}var g=b.data.geometry.coordinates[0],h=0,j=JSON.parse(b.data.properties.levels);j.forEach(function(a){h+=a});for(var l=b._three._threebox.distaneToWorld(h),m={steps:10,units:"kilometers",properties:{}},o=[],p=[],r=[],n=[],q=[],t=[],s=
[],u=0;u<g.length;u++){var v=[],w=b._three._threebox.projectToWorld(g[u]);p.push(w.clone());var y=w.clone();y.z=l;r.push(y.clone());v.push(w.clone());v.push(y.clone());n.push(v);q.push(w.clone());t.push(w.clone());s.push(w.clone());v=turf.circle(g[u],2.0E-4,m);o.push(v)}new THREE.Shape(s);g=b._createGeoExtrudeGeometry(s,{amount:1.0E-12,bevelEnabled:!1});m=new THREE.MeshPhongMaterial({color:new THREE.Color(this.roofColor),transparent:!0,opacity:this.roofOpacity});g.translate(0,0,l+1.0E-6);g=new THREE.Mesh(g,
m);this.isRoofVisible&&c.add(g);m=new THREE.MeshPhongMaterial({color:this.topLineColor,transparent:!0,opacity:this.topLineOpacity});g=new THREE.MeshPhongMaterial({color:this.pillarColor,transparent:!0,opacity:this.pillarOpacity});p=b._createLineGeometry(p);n=b._createLineGeometry(r);r=p.clone();(new THREE.Line(p,m)).name="linebase";p=new THREE.Line(n,m);p.name="lineUp";c.add(p);l={pillar:{amount:l,bevelEnabled:!1},floor:{amount:1.0E-12,bevelEnabled:!1}};o=b._createPillarMesh(o,l.pillar,g);o.name=
"pillar";this.isPillarVisible&&c.add(o);o=new THREE.MeshPhongMaterial({color:this.floorLinesColor,transparent:!0,opacity:this.floorLinesOpacity});q=b._createGeoExtrudeGeometry(q,l.floor);for(p=l=0;p<j.length;p++)g=b._three._threebox.distaneToWorld(l),n=new THREE.MeshPhongMaterial({color:b.originalFloorColor,transparent:!0,opacity:b.originalFloorOpacity,specular:6454209,shininess:100}),m=new THREE.MeshPhongMaterial({color:b.originalWallColor,transparent:!0,opacity:b.originalWallOpacity,specular:16776960,
shininess:100,side:THREE.DoubleSide}),s=q.clone(),s.translate(0,0,g),n=new THREE.Mesh(s,n),n.userData.attributes={OriginalData:b.data,Layer:b,level:p+1,tag:"__floor__"},n.userData.tag="__floor__",n.name="this is "+(p+1)+" floor",c.add(n),b.isFloorLinesVisible&&(n=r.clone(),n.translate(0,0,g),n=new THREE.Line(n,o),c.add(n)),n={amount:b._three._threebox.distaneToWorld(j[p]),bevelEnabled:!1},n=b._createSideGeometry(t,n),n.translate(0,0,g),g=new THREE.Mesh(n,m),g.userData.attributes={OriginalData:b.data,
Layer:b,level:p+1,tag:"__wall__"},g.userData.tag="__wall__",g.name="this is "+(p+1)+" wall",c.add(g),l+=j[p];var j=new THREE.MeshBasicMaterial({color:16777215,transparent:!0,opacity:1}),t=new THREE.SphereBufferGeometry(0.001,32,16),C=new THREE.Mesh(t,j);C.position.copy(b._three._threebox.projectToWorld([e[0],e[1],0]));C.name="sphereTarget";C.scale.copy((new THREE.Vector3(1,1,1)).multiplyScalar(b._three._threebox.projectedUnitsPerMeterInScale(e[1])));b.lightShown&&d.add(C);j=new THREE.PointLight(new THREE.Color(b.lightColor),
200,0);j.matrixWorldNeedsUpdate=!0;C.add(j);b._three._threebox.addGeoreferencedMeshToWorld2(c);b._meshes.push(c);b._three._threebox.addGeoreferencedMesh(d);b._meshes.push(d);var d=b._three._threebox.projectToWorld([e[0],e[1],h]),j=b._three._threebox.projectToWorld([e[0],e[1],0]),B=new THREE.LineCurve3(j,d),A=0;b.lightShown&&window.requestAnimationFrame(function(b){return a(b,0)})}}});
...
a[0]=1;a[1]=0;a[2]=0;a[3]=0;a[4]=0;a[5]=1;a[6]=0;a[7]=0;a[8]=0;a[9]=0;a[10]=1;a[11]=0;a[12]=0;a[13]=0;a[14]=0;a[15]=1;return a},identity:function(a){a[0]=1;a[1]=0;a[2]=0;a[3]=0;a[4]=0;a[5]=1;a[6]=0;a[7]=0;a[8]=0;a[9]=0;a[10]=1;a[11]=0;a[12]=0;a[13]=0;a[14]=0;a[15]=1;return a},translate:function(a,b,c){var d=c[0],e=c[1],c=c[2],f,g,h,j,l,m,o,p,r,n,q,t;b===a?(a[12]=b[0]*d+b[4]*e+b[8]*c+b[12],a[13]=b[1]*d+b[5]*e+b[9]*c+b[13],a[14]=b[2]*d+b[6]*e+b[10]*c+b[14],a[15]=b[3]*d+b[7]*e+b[11]*c+b[15]):(f=b[0],
g=b[1],h=b[2],j=b[3],l=b[4],m=b[5],o=b[6],p=b[7],r=b[8],n=b[9],q=b[10],t=b[11],a[0]=f,a[1]=g,a[2]=h,a[3]=j,a[4]=l,a[5]=m,a[6]=o,a[7]=p,a[8]=r,a[9]=n,a[10]=q,a[11]=t,a[12]=f*d+l*e+r*c+b[12],a[13]=g*d+m*e+n*c+b[13],a[14]=h*d+o*e+q*c+b[14],a[15]=j*d+p*e+t*c+b[15]);return a},scale:function(a,b,c){var d=c[0],e=c[1],c=c[2];a[0]=b[0]*d;a[1]=b[1]*d;a[2]=b[2]*d;a[3]=b[3]*d;a[4]=b[4]*e;a[5]=b[5]*e;a[6]=b[6]*e;a[7]=b[7]*e;a[8]=b[8]*c;a[9]=b[9]*c;a[10]=b[10]*c;a[11]=b[11]*c;a[12]=b[12];a[13]=b[13];a[14]=b[14];
a[15]=b[15];return a},multiply:function(a,b,c){var d=b[0],e=b[1],f=b[2],g=b[3],h=b[4],j=b[5],l=b[6],m=b[7],o=b[8],p=b[9],r=b[10],n=b[11],q=b[12],t=b[13],s=b[14],b=b[15],u=c[0],v=c[1],w=c[2],y=c[3];a[0]=u*d+v*h+w*o+y*q;a[1]=u*e+v*j+w*p+y*t;a[2]=u*f+v*l+w*r+y*s;a[3]=u*g+v*m+w*n+y*b;u=c[4];v=c[5];w=c[6];y=c[7];a[4]=u*d+v*h+w*o+y*q;a[5]=u*e+v*j+w*p+y*t;a[6]=u*f+v*l+w*r+y*s;a[7]=u*g+v*m+w*n+y*b;u=c[8];v=c[9];w=c[10];y=c[11];a[8]=u*d+v*h+w*o+y*q;a[9]=u*e+v*j+w*p+y*t;a[10]=u*f+v*l+w*r+y*s;a[11]=u*g+v*m+
w*n+y*b;u=c[12];v=c[13];w=c[14];y=c[15];a[12]=u*d+v*h+w*o+y*q;a[13]=u*e+v*j+w*p+y*t;a[14]=u*f+v*l+w*r+y*s;a[15]=u*g+v*m+w*n+y*b;return a},perspective:function(a,b,c,d,e){var b=1/Math.tan(b/2),f=1/(d-e);a[0]=b/c;a[1]=0;a[2]=0;a[3]=0;a[4]=0;a[5]=b;a[6]=0;a[7]=0;a[8]=0;a[9]=0;a[10]=(e+d)*f;a[11]=-1;a[12]=0;a[13]=0;a[14]=2*e*d*f;a[15]=0;return a},rotateX:function(a,b,c){var d=Math.sin(c),c=Math.cos(c),e=b[4],f=b[5],g=b[6],h=b[7],j=b[8],l=b[9],m=b[10],o=b[11];b!==a&&(a[0]=b[0],a[1]=b[1],a[2]=b[2],a[3]=
...
b){var c=b[0],d=b[1],e=b[2],f=b[3],g=b[4],h=b[5],j=b[6],l=b[7],m=b[8],o=b[9],p=b[10],r=b[11],n=b[12],q=b[13],t=b[14],s=b[15],u=c*h-d*g,v=c*j-e*g,w=c*l-f*g,y=d*j-e*h,C=d*l-f*h,B=e*l-f*j,A=m*q-o*n,z=m*t-p*n,x=m*s-r*n,E=o*t-p*q,F=o*s-r*q,G=p*s-r*t,D=u*G-v*F+w*E+y*x-C*z+B*A;if(!D)return null;D=1/D;a[0]=(h*G-j*F+l*E)*D;a[1]=(e*F-d*G-f*E)*D;a[2]=(q*B-t*C+s*y)*D;a[3]=(p*C-o*B-r*y)*D;a[4]=(j*x-g*G-l*z)*D;a[5]=(c*G-e*x+f*z)*D;a[6]=(t*w-n*B-s*v)*D;a[7]=(m*B-p*w+r*v)*D;a[8]=(g*F-h*x+l*A)*D;a[9]=(d*x-c*F-f*A)*
D;a[10]=(n*C-q*w+s*u)*D;a[11]=(o*w-m*C-r*u)*D;a[12]=(h*z-g*E-j*A)*D;a[13]=(c*E-d*z+e*A)*D;a[14]=(q*v-n*y-t*u)*D;a[15]=(m*y-o*v+p*u)*D;return a},ortho:function(a,b,c,d,e,f,g){var h=1/(b-c),j=1/(d-e),l=1/(f-g);a[0]=-2*h;a[1]=0;a[2]=0;a[3]=0;a[4]=0;a[5]=-2*j;a[6]=0;a[7]=0;a[8]=0;a[9]=0;a[10]=2*l;a[11]=0;a[12]=(b+c)*h;a[13]=(e+d)*j;a[14]=(g+f)*l;a[15]=1;return a}}}});
...
c:this.value},b.formatter).split("<br/>"),o=m.length,p=0;p<o;p++){var r=c.measureText(m[p]).width;r>j&&(j=r);c.fillText(m[p],this.x,this.y-((o-1)/2-p)*(this.radius*2+5))}else j=c.measureText("\uffe5"+this.value).width;c.rect(this.x-j/2,this.y-l/2,j,l)}break;case "fire":c.rect(this.x-this.size/2,this.y-this.size/2,this.size,this.size);break;case "icon":c.rect(this.x-b.symbolWidth/2,this.y-b.symbolHeight/2,b.symbolWidth,b.symbolHeight)}if(c.isPointInPath(d,h))a.hoveredMarkPoint=this};this.MarkPoint.prototype.draw1=
...
parseInt(a.x)&&parseInt(g[b].y)===parseInt(a.y))return g[b];return null},o=function(){a.fillStyle=r.color;a.beginPath();switch(e){case "square":a.globalCompositeOperation="lighter";a.rect(r.x-f/2,r.y-f/2,f,f);break;case "square2":a.rect(r.x-f/2,r.y-f/2,f,f);break;case "round":a.globalCompositeOperation="lighter",a.arc(r.x,r.y,f/2,0,Math.PI*2,!1)}a.fill()},p=this.markPoints.length;for(this.markPoints=this.markPoints.sort(function(a,b){var c=a.value,d=b.value;return c>d?-1:c<d?1:0});p--;){var r;r=this.markPoints[p];
...
this.parts.length;c++)for(e=0;e<this.parts[c].cubes.length;e++){var s=0;for(f=e;f>0;f--)s+=this.parts[c].cubes[f-1].height;this.parts[c].cubes[e].vertexs=[[m+c*(g+j),l-s],[m+c*(g+j)+h*Math.cos(b),l-h*Math.sin(b)-s],[m+c*(g+j)+h*Math.cos(b)-g,l-h*Math.sin(b)-s],[m+c*(g+j)-g,l-s]]}break;case "pie":var u,v,w,y,C,B=Math.PI*2/50;for(c=0;c<1;c++){f=this.parts[c].size/2;m=f>g?f-g:0;s=new this.Point(0,0,-h/2);u=new this.Point(0,0,h/2);v=[];w=[];y=[];C=[];for(l=0;l<=Math.PI*2;)v.push(new this.Point(f*Math.cos(l),
f*Math.sin(l),-h/2)),w.push(new this.Point(f*Math.cos(l),f*Math.sin(l),h/2)),m>0&&(y.push(new this.Point(m*Math.cos(l),m*Math.sin(l),-h/2)),C.push(new this.Point(m*Math.cos(l),m*Math.sin(l),h/2))),l+=B;v.push(new this.Point(f,0,-h/2));w.push(new this.Point(f,0,h/2));m>0&&(y.push(new this.Point(m,0,-h/2)),C.push(new this.Point(m,0,h/2)));e=Math.cos(b);var A=Math.sin(b),z;l=s.y*e-s.z*A;z=s.z*e+s.y*A;s.y=l;s.z=z;l=u.y*e-u.z*A;z=u.z*e+u.y*A;u.y=l;u.z=z;for(f=0;f<50;f++)if(l=v[f].y*e-v[f].z*A,z=v[f].z*
e+v[f].y*A,v[f].y=l,v[f].z=z,l=w[f].y*e-w[f].z*A,z=w[f].z*e+w[f].y*A,w[f].y=l,w[f].z=z,m>0)l=y[f].y*e-y[f].z*A,z=y[f].z*e+y[f].y*A,y[f].y=l,y[f].z=z,l=C[f].y*e-C[f].z*A,z=C[f].z*e+C[f].y*A,C[f].y=l,C[f].z=z;for(e=0;e<this.parts[c].sectors.length;e++){l=this.parts[c].sectors[e].angle;A=0;for(f=e;f>0;f--)A+=this.parts[c].sectors[f-1].angle;f=A+l;this.parts[c].sectors[e].startIndex=e===0?Math.round(A/B):this.parts[c].sectors[e-1].endIndex;this.parts[c].sectors[e].endIndex=Math.round(f/B)}this.parts[c].uCentrePoint=
s;this.parts[c].tCentrePoint=u;this.parts[c].oUnderPoints=v;this.parts[c].oTopPoints=w;if(m>0)this.parts[c].iUnderPoints=y,this.parts[c].iTopPoints=C}}};this.MarkPoint.prototype.update=function(){this.energy+=this.velocity;this.energy>1&&(this.energy=1);a.DRAWDONE=this.energy===1};this.MarkPoint.prototype.isPointInPath=function(c,e,f){var h,j,l;switch(d){case "column":for(h=0;h<this.parts.length;h++)for(j=0;j<this.parts[h].cubes.length;j++)if(this.parts[h].cubes[j].height>0&&(c.beginPath(),c.moveTo(this.parts[h].cubes[j].vertexs[0][0],
...
j;return}break;case "pie":for(h=0;h<1;h++){var s=this.parts[h].tCentrePoint,u=this.parts[h].oUnderPoints,v=this.parts[h].oTopPoints,w=this.parts[h].iUnderPoints,y=this.parts[h].iTopPoints,C=!1;this.parts[h].size/2>g&&(C=!0);var B=Math.round(25),A=[],z=[],x=[];l=[];for(j=0;j<this.parts[h].sectors.length;j++)A.push(this.parts[h].sectors[j].angle),z.push(this.parts[h].sectors[j].startIndex),x.push(Math.round(this.parts[h].sectors[j].endIndex*this.energy)),l.push(b[j]?b[j]:m.color);for(j=A.length-1;j>=
0;j--)if(A[j]>0){c.beginPath();if(z[j]>B){c.moveTo(v[x[j]].get2d().x,v[x[j]].get2d().y);for(l=x[j]-1;l>=z[j];l--)c.lineTo(v[l].get2d().x,v[l].get2d().y);if(C){c.lineTo(y[z[j]].get2d().x,y[z[j]].get2d().y);for(l=z[j];l<=x[j];l++)c.lineTo(w[l].get2d().x,w[l].get2d().y);c.lineTo(y[x[j]].get2d().x,y[x[j]].get2d().y)}else c.lineTo(s.get2d().x,s.get2d().y)}else if(x[j]<B){c.moveTo(u[x[j]].get2d().x,u[x[j]].get2d().y);for(l=x[j]-1;l>=z[j];l--)c.lineTo(u[l].get2d().x,u[l].get2d().y);c.lineTo(v[z[j]].get2d().x,
v[z[j]].get2d().y);if(C)for(l=z[j];l<=x[j];l++)c.lineTo(y[l].get2d().x,y[l].get2d().y);else c.lineTo(s.get2d().x,s.get2d().y);c.lineTo(v[x[j]].get2d().x,v[x[j]].get2d().y)}else{c.moveTo(v[x[j]].get2d().x,v[x[j]].get2d().y);for(l=x[j]-1;l>=B;l--)c.lineTo(v[l].get2d().x,v[l].get2d().y);for(l=B;l>=z[j];l--)c.lineTo(u[l].get2d().x,u[l].get2d().y);c.lineTo(v[z[j]].get2d().x,v[z[j]].get2d().y);if(C){for(l=z[j];l<=B;l++)c.lineTo(y[l].get2d().x,y[l].get2d().y);for(l=B;l<=x[j];l++)c.lineTo(w[l].get2d().x,
...
this.y+(h/2+5)*Math.sin(e)),c.lineTo(this.x+(h/2+5)*Math.cos(e),this.y-(h/2+5)*Math.sin(e)),c.closePath(),c.stroke());break;case "pie":c.globalAlpha=l;for(f=0;f<1;f++){t=this.parts[f].uCentrePoint;var s=this.parts[f].tCentrePoint,u=this.parts[f].oUnderPoints,v=this.parts[f].oTopPoints,w=this.parts[f].iUnderPoints,y=this.parts[f].iTopPoints,C=!1;this.parts[f].size/2>g&&(C=!0);var B=Math.round(25),A=[],z=[],x=[],E=[];for(n=0;n<this.parts[f].sectors.length;n++)A.push(this.parts[f].sectors[n].angle),
z.push(this.parts[f].sectors[n].startIndex),x.push(Math.round(this.parts[f].sectors[n].endIndex*this.energy)),E.push(b[n]?b[n]:m.color);for(n=A.length-1;n>=0;n--)A[n]>0&&a.hoveredMarkPoint===this&&f===this.hActive&&n===this.vActive&&(E[n]=GeoGlobe.Util.getShadeColor(E[n],10));for(n=A.length-1;n>=0;n--)if(A[n]>0){c.globalCompositeOperation="destination-over";c.beginPath();c.moveTo(u[x[n]].get2d().x,u[x[n]].get2d().y);for(q=x[n]-1;q>=z[n];q--)c.lineTo(u[q].get2d().x,u[q].get2d().y);if(C)for(q=z[n];q<=
x[n];q++)c.lineTo(w[q].get2d().x,w[q].get2d().y);else c.lineTo(t.get2d().x,t.get2d().y);c.fillStyle=E[n];c.fill()}if(e!==0){for(n=A.length-1;n>=0;n--)if(A[n]>0)c.globalCompositeOperation="source-over",c.beginPath(),c.moveTo(u[x[n]].get2d().x,u[x[n]].get2d().y),C?(c.lineTo(w[x[n]].get2d().x,w[x[n]].get2d().y),c.lineTo(y[x[n]].get2d().x,y[x[n]].get2d().y)):(c.lineTo(t.get2d().x,t.get2d().y),c.lineTo(s.get2d().x,s.get2d().y)),c.lineTo(v[x[n]].get2d().x,v[x[n]].get2d().y),c.moveTo(u[z[n]].get2d().x,u[z[n]].get2d().y),
C?(c.lineTo(w[z[n]].get2d().x,w[z[n]].get2d().y),c.lineTo(y[z[n]].get2d().x,y[z[n]].get2d().y)):(c.lineTo(t.get2d().x,t.get2d().y),c.lineTo(s.get2d().x,s.get2d().y)),c.lineTo(v[z[n]].get2d().x,v[z[n]].get2d().y),c.fillStyle=GeoGlobe.Util.getShadeColor(E[n],-20),c.fill();if(C)for(n=A.length-1;n>=0;n--)if(A[n]>0){c.globalCompositeOperation="source-over";c.beginPath();c.moveTo(w[x[n]].get2d().x,w[x[n]].get2d().y);if(z[n]<B&&x[n]>B){for(q=x[n]-1;q>=B;q--)c.lineTo(w[q].get2d().x,w[q].get2d().y);for(q=
B;q<=x[n];q++)c.lineTo(y[q].get2d().x,y[q].get2d().y);c.moveTo(w[z[n]].get2d().x,w[z[n]].get2d().y);for(q=z[n]+1;q<=B;q++)c.lineTo(w[q].get2d().x,w[q].get2d().y);for(q=B;q>=z[n];q--)c.lineTo(y[q].get2d().x,y[q].get2d().y)}else{for(q=x[n]-1;q>=z[n];q--)c.lineTo(w[q].get2d().x,w[q].get2d().y);for(q=z[n];q<=x[n];q++)c.lineTo(y[q].get2d().x,y[q].get2d().y)}c.fillStyle=GeoGlobe.Util.getShadeColor(E[n],-20);c.fill()}for(n=A.length-1;n>=0;n--)if(A[n]>0){if(z[n]<B&&x[n]>B){c.globalCompositeOperation="destination-over";
c.beginPath();c.moveTo(u[x[n]].get2d().x,u[x[n]].get2d().y);for(q=x[n]-1;q>=B;q--)c.lineTo(u[q].get2d().x,u[q].get2d().y);for(q=B;q<=x[n];q++)c.lineTo(v[q].get2d().x,v[q].get2d().y);c.fillStyle=GeoGlobe.Util.getShadeColor(E[n],-20);c.fill();c.globalCompositeOperation="source-over";c.beginPath();c.moveTo(u[z[n]].get2d().x,u[z[n]].get2d().y);for(q=z[n]+1;q<=B;q++)c.lineTo(u[q].get2d().x,u[q].get2d().y);for(q=B;q>=z[n];q--)c.lineTo(v[q].get2d().x,v[q].get2d().y)}else{c.globalCompositeOperation="source-over";
c.beginPath();c.moveTo(u[x[n]].get2d().x,u[x[n]].get2d().y);for(q=x[n]-1;q>=z[n];q--)c.lineTo(u[q].get2d().x,u[q].get2d().y);for(q=z[n];q<=x[n];q++)c.lineTo(v[q].get2d().x,v[q].get2d().y)}c.fillStyle=GeoGlobe.Util.getShadeColor(E[n],-20);c.fill()}for(n=A.length-1;n>=0;n--)if(A[n]>0){c.globalCompositeOperation="source-over";c.beginPath();c.moveTo(v[x[n]].get2d().x,v[x[n]].get2d().y);for(q=x[n]-1;q>=z[n];q--)c.lineTo(v[q].get2d().x,v[q].get2d().y);if(C)for(q=z[n];q<=x[n];q++)c.lineTo(y[q].get2d().x,
y[q].get2d().y);else c.lineTo(s.get2d().x,s.get2d().y);c.fillStyle=E[n];c.fill()}}}}}},_initCanvas:function(){this.canvas=[];this.canvasContext=[];this.canvas.push(document.createElement("canvas"));this.canvasContext.push(this.canvas[0].getContext("2d"));this.cacheCanvas=[];this.cacheCanvasContext=[];this.cacheCanvas.push(document.createElement("canvas"));this.cacheCanvasContext.push(this.cacheCanvas[0].getContext("2d"));this.canvas[0].style.position="absolute";this.div.appendChild(this.canvas[0])},
...
g;this.legendCanvas.height=b;var h=function(b,c,d){this.legendCanvas.width=g;for(var e=!1,f,h,n=0,q=this._legend.vLegendLabels.length;n<q;n++){h=0;for(var t=this._legend.col;h<t;h++)if(n%this._legend.col===h){f=10+this._legend.itemWidth/2;for(var s=0;s<h;s++)f+=a[s]+10}h=10+this._legend.itemHeight/2+((this._legend.itemHeight<this._legend.textStyle.fontSize?this._legend.textStyle.fontSize:this._legend.itemHeight)+10)*~~(n/this._legend.col);s=t=!1;this.legendCanvasContext.beginPath();this.legendCanvasContext.rect(f-
...
l=void 0,m=void 0,o=void 0,p=void 0,r=void 0,n=void 0;this.markPoints.forEach(function(a){l=Math.floor((a.mercator[0]-e._markPointSWMercator[0])/h);m=Math.floor((a.mercator[1]-e._markPointSWMercator[1])/h);o=l+"_"+m;if(g[o])g[o].count++,g[o].sumLonLat[0]+=a.lon,g[o].sumLonLat[1]+=a.lat,g[o].children.push(a);else if(g[o]={key:o,count:1,sumLonLat:[a.lon,a.lat],children:[a]},b>f)p=Math.floor((a.mercator[0]-e._markPointSWMercator[0])/j),r=Math.floor((a.mercator[1]-e._markPointSWMercator[1])/j),n=p+"_"+
...
a[d];var e={},f={},g=void 0,h=void 0;for(d in a)if(g=a[d],!f[d])for(var h=d.split("_"),j=+h[0],l=+h[1],m=-1;m<=1;m++)for(var o=-1;o<=1;o++)if(!(m===0&&o===0)){var p=j+m+"_"+(l+o);if(h=a[p]){var r=GeoGlobe.Util.transferToMercator([g.sumLonLat[0]/g.count,g.sumLonLat[1]/g.count]),n=GeoGlobe.Util.transferToMercator([h.sumLonLat[0]/h.count,h.sumLonLat[1]/h.count]),q=r[0]-n[0],r=r[1]-n[1];Math.sqrt(q*q+r*r)<=b&&(e[d]=e[d]||[],e[d].push(h),f[p]=1)}}for(var t in e)if(d=a[t]){f=e[t];for(g=0;g<f.length;g++)if(a[f[g].key])d.count+=
...
h+e[0]*g+f[0]*b;c=c[1]*j+d[1]*h+e[1]*g+f[1]*b;return[a,c,Math.sqrt(a*a+c*c)]},r=function(a,b){var c=a.data,d=b.data;return{header:a.header,data:function(a){return[c[a],d[a]]},interpolate:p}},n=function(a){var b=null,c=null;a.forEach(function(a){switch(a.header.parameterCategory+","+a.header.parameterNumber){case "2,2":b=a;break;case "2,3":c=a}});return r(b,c)},q=function(a){return a!==null&&a!==void 0},t=function(a,b,c){function d(b,c){var e=a[Math.round(b)];return e&&e[Math.round(c)]||l}d.release=
...
4))},y=function(a,c,d){if(b._mapCRS)var e=d.width/(d.east-d.west),f=d.height/(d.north-d.south),c=(u(c)-d.west)*e,a=(d.north-u(a))*f;else var f=w(d.south),g=w(d.north),e=d.width/(d.east-d.west),f=d.height/(g-f),a=w(u(a)),c=(u(c)-d.west)*e,a=(g-a)*f;return[c,a]},C=function(a,d,e){var f=[],g=a.x;(function I(){for(var h=Date.now();g<a.width;){for(var j=g,l=[],n=a.y;n<=a.yMax;n+=2){var p,q=j,r=n;p=d;if(b._mapCRS)var s=p.east-p.west,u=p.north-p.south,q=v(p.west)+q/p.width*v(s),r=v(p.north)-r/p.height*v(u);
else s=p.east-p.west,u=p.width/v(s)*360/(2*Math.PI),r=180/Math.PI*(2*Math.atan(Math.exp((p.height+u/2*Math.log((1+Math.sin(p.south))/(1-Math.sin(p.south)))-r)/u))-Math.PI/2),q=v(p.west)+q/p.width*v(s);p=[q,r];s=p[0];u=p[1];if(isFinite(s)&&(p=z(s,u))){var q=p[0]*c,r=p[1]*c,w=s,x=u,s=j,u=n,A=d,B=w<0?o:-o,C=x<0?o:-o,H=y(x,w+B,A),w=y(x+C,w,A),x=Math.cos(x/360*m);b._mapCRS&&(x=1);s=[(H[0]-s)/B/x,(H[1]-u)/B/x,(w[0]-s)/C,(w[1]-u)/C];p[0]=s[0]*q+s[2]*r;p[1]=s[1]*q+s[3]*r;l[n+1]=l[n]=p}}f[j+1]=f[j]=l;g+=2;
if(Date.now()-h>1E3){setTimeout(I,25);return}}t(f,a,e)})()},B=function(b,c){function l(a){return a.charAt(0)=="#"?a.substring(1,7):a}function m(){p.forEach(function(a){a.length=0});r.forEach(function(a){if(a.age>e)c.randomize(a).age=0;var b=a.x,d=a.y,f=c(b,d),g=f[2];g===null?a.age=e:(b+=f[0],d+=f[1],c(b,d)[2]!==null?(a.xt=b,a.yt=d,p[o.indexFor(g)].push(a)):(a.x=b,a.y=d));a.age+=1})}function n(){var a=t.globalCompositeOperation;t.globalCompositeOperation="destination-in";t.fillRect(b.x,b.y,b.width,
...
q=Math.round(b.width*b.height*g);/android|blackberry|iemobile|ipad|iphone|ipod|opera mini|webos/i.test(navigator.userAgent)&&(q*=0.075);for(var r=[],s=0;s<q;s++)r.push(c.randomize({age:Math.floor(Math.random()*e)+0}));var t=a.canvasContext;t.lineWidth=f;t.fillStyle="rgba(0, 0, 0, 0.97)";(function J(){try{x.timer=setTimeout(function(){requestAnimationFrame(J);m();n()},50)}catch(a){console.error(a)}})()},A=function(){x.field&&x.field.release();x.timer&&clearTimeout(x.timer)},z=function(a){var b=n(a),
c=b.header,d=c.lo1,e=c.la1,f=c.dx,g=c.dy,a=c.nx,h=c.ny,j=new Date(c.refTime);j.setHours(j.getHours()+c.forecastTime);for(var l=[],c=0,j=Math.floor(a*f)>=360,m=0;m<h;m++){for(var o=[],p=0;p<a;p++,c++)o[p]=b.data(c);j&&o.push(o[0]);l[m]=o}return function(a,c){var h=(a-d-360*Math.floor((a-d)/360))/f,j=(e-c)/g,m=Math.floor(h),n=m+1,o=Math.floor(j),p=o+1,r;if(r=l[o]){var t=r[m],s=r[n];if(q(t)&&q(s)&&(r=l[p]))if(p=r[m],n=r[n],q(p)&&q(n))return b.interpolate(h-m,j-o,t,s,p,n)}return null}}(a.data),x={start:function(a,
b,c,d){d={south:u(d[0][1]),north:u(d[1][1]),east:u(d[1][0]),west:u(d[0][0]),width:b,height:c};A();C(s(a,b,c),d,function(a,b){x.field=b;B(a,b)})},stop:A};return x}},render:function(){this._init_RendererOptions();this._initWindy();this.setData(this.data);this.setVisible(this.visibility)},draw:function(){setTimeout(function(){this.windy.start([[0,0],this.size],this.size[0],this.size[1],this.bounds)}.bind(this),1)},onMove:function(){this.stopDraw()},onMoveEnd:function(){this.redraw()},onResize:function(){this.width=
...
c)/d;return c+(e-c)/d*(1.5*(a/d)-0.5*Math.pow(a/d,3))};a.train=function(a,c,d,e,f,g){var h={t:a,x:c,y:d,nugget:0,range:0,sill:0,A:1/3,n:0};switch(e){case "gaussian":h.model=kriging_variogram_gaussian;break;case "exponential":h.model=kriging_variogram_exponential;break;case "spherical":h.model=kriging_variogram_spherical}var j,l,m,o,p=a.length,r=Array((p*p-p)/2);for(m=j=0;j<p;j++)for(l=0;l<j;l++,m++)r[m]=Array(2),r[m][0]=Math.pow(Math.pow(c[j]-c[l],2)+Math.pow(d[j]-d[l],2),0.5),r[m][1]=Math.abs(a[j]-
a[l]);r.sort(function(a,b){return a[0]-b[0]});h.range=r[(p*p-p)/2-1][0];var n=(p*p-p)/2>30?30:(p*p-p)/2,q=h.range/n,t=[0].rep(n),s=[0].rep(n);if(n<30)for(o=0;o<n;o++)t[o]=r[o][0],s[o]=r[o][1];else{for(o=m=l=j=0;j<n&&l<(p*p-p)/2;j++,m=0){for(;r[l][0]<=(j+1)*q;)if(t[o]+=r[l][0],s[o]+=r[l][1],l++,m++,l>=(p*p-p)/2)break;m>0&&(t[o]/=m,s[o]/=m,o++)}if(o<2)return h}p=o;h.range=t[p-1]-t[0];m=[1].rep(2*p);l=Array(p);o=h.A;for(j=0;j<p;j++){switch(e){case "gaussian":m[j*2+1]=1-Math.exp(-(1/o)*Math.pow(t[j]/
h.range,2));break;case "exponential":m[j*2+1]=1-Math.exp(-(1/o)*t[j]/h.range);break;case "spherical":m[j*2+1]=1.5*(t[j]/h.range)-0.5*Math.pow(t[j]/h.range,3)}l[j]=s[j]}j=kriging_matrix_transpose(m,p,2);e=kriging_matrix_multiply(j,m,2,p,2);e=kriging_matrix_add(e,kriging_matrix_diag(1/g,2),2,2);g=e.slice(0);kriging_matrix_chol(e,2)?kriging_matrix_chol2inv(e,2):(kriging_matrix_solve(g,2),e=g);p=kriging_matrix_multiply(kriging_matrix_multiply(e,j,2,2,p),l,2,p,1);h.nugget=p[0];h.sill=p[1]*h.range+h.nugget;
